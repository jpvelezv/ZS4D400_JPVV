...
CLASS lcl_flights DEFINITION.
  ...
  PRIVATE SECTION.
    DATA connections TYPE SORTED TABLE OF zs4djpvv_flights WITH NON-UNIQUE KEY carrier_id connection_Id flight_date.
    DATA connections_sk TYPE SORTED TABLE OF zs4djpvv_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date
...
CLASS lcl_flights IMPLEMENTATION.
  ...
  METHOD constructor.
    SELECT FROM zs4djpvv_flights FIELDS * INTO TABLE @connections.
    SELECT FROM zs4djpvv_flights FIELDS * INTO TABLE @connections_sk.
  ENDMETHOD.
