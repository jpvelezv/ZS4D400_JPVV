...
CLASS lcl_flights DEFINITION.
  ...
  PRIVATE SECTION.
    DATA standard_table TYPE STANDARD TABLE OF zs4djpvv_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date.
    DATA sorted_table TYPE SORTED TABLE OF zs4djpvv_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date.
    DATA hashed_table TYPE HASHED TABLE OF zs4djpvv_flights WITH UNIQUE KEY carrier_id connection_id flight_date.
...
CLASS lcl_flights IMPLEMENTATION.
  ...
  METHOD constructor.
    SELECT FROM zs4djpvv_flights FIELDS * INTO TABLE @standard_table.
    SELECT FROM zs4djpvv_flights FIELDS * INTO TABLE @sorted_table.
    SELECT FROM zs4djpvv_flights FIELDS * INTO TABLE @hashed_table.
